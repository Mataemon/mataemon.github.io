 ...
 ************************************************/
     ...
     (async function(){
         tf.wasm.setWasmPaths({
             'tfjs-backend-wasm.wasm' : 'js/tensorflow/tfjs-backend-wasm.wasm',
             'tfjs-backend-wasm-simd.wasm' : 'js/tensorflow/tfjs-backend-wasm-simd.wasm',
             'tfjs-backend-wasm-threaded-simd.wasm' : 'js/tensorflow/tfjs-backend-wasm-threaded-simd.wasm',
         });
     ...
     let PERFORM_ACTION = function(action, name){
         ...
         if(action === "start"){
             ...
             if(name !== "back"){ /* At the beginning we had 3 videos : front, back, face. An update merged the front and back video together, we dont have to start at the begin of the back video but we must keep all other mechanisms (getting timestamps ...) */
                 utils.record(name, null, {"video_dimension" : [ELT_VIDEO.videoWidth, ELT_VIDEO.videoHeight]}, null);
             }
